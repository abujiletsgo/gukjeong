    ...
    <div className="card">
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-accent">{legislator.aiActivityScore || '-'}</div>
          <div className="text-xs text-gray-500">활동 점수</div>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-4 mt-4 text-center">
        <div>
          <div className="text-lg font-bold">{legislator.attendanceRate ? `${legislator.attendanceRate}%` : '-'}</div>
          <div className="text-xs text-gray-500">출석률</div>
        </div>
        <div>
          <div className="text-lg font-bold">{legislator.billsProposedCount || 0}</div>
          <div className="text-xs text-gray-500">발의 법안</div>
        </div>
        <div>
          <div className="text-lg font-bold">{legislator.consistencyScore ? `${legislator.consistencyScore}%` : '-'}</div>
