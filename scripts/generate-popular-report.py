...
THEMES = [
    {
        "id": "cheongdo_county_probe",
        "label": "김하수 전 청도군수 인사비리 의혹과 청도군 조달 구조",
        "category": "지방권력",
        "trace_strength": "강함",
        "caveat": (
            "승진 청탁·금품 수수 의혹(경찰 수사 대상)과 아래 조달 패턴은 별개 사안이다. "
            "흔적은 청도군 조달 데이터 전반의 구조적 이상 신호이지 고인의 혐의를 "
            "뒷받침하는 증거가 아니며, 수사는 당사자 사망으로 종결될 가능성이 크다."
        ),
        "news_match": re.compile(r"김하수|청도군수|청도군.*(수사|청탁|금품|숨진|사망)"),
        "audit_query": re.compile(r"."),
        "inst_focus": re.compile(r"청도군"),
        "auditable_angle": (
            "단체장 인사 전횡 의혹이 제기된 지자체는 조달에서도 내부 통제 실패가 함께 "
            "나타나는 경우가 많다. 청도군은 국정투명 감사 데이터에서 동일 주소를 "
            "공유하는 건설업체군, 동일 업체 반복 낙찰, 재공고 후 동일 업체 재낙찰 등 "
            "8가지 유형의 의심 패턴이 동시에 감지된 기관이다. 인사 비리 의혹과 조달 "
            "이상 신호가 같은 시기 같은 조직에서 나온 만큼, 내부 통제 전반에 대한 "
            "점검 가치가 있다."
        ),
        "what_to_check": [
            "동일 주소(각북면 헐티로 일대) 3개 건설업체의 실체와 상호 들러리 입찰 여부",
            "생활폐기물 수집·운반 대행 용역의 재공고-동일업체 반복 낙찰 구조",
            "군수 재임 기간 중 특정 업체 수주 집중도의 변화 추이",
            "승진 청탁 의혹 관련 인물과 반복 수주 업체 간 인적 연결 고리",
        ],
        "citizen_impact": (
            "인구 4만 소규모 군의 조달 예산은 도로·폐기물·상하수도 등 주민 생활 "
            "인프라와 직결된다. 내부 통제가 무너진 조직에서는 인사와 계약이 함께 "
            "거래 대상이 되기 쉽다."
        ),
    },
    {
...
NON_PROCUREMENT = re.compile(
    ...
    r"우승|황금사자기|실종|보완수사권|홈플러스|코스피|사이드카|윔블던|PGA|"
    r"아비뇽|노벨|당권|전당대회|수출|자작극|혐의 인정)"
)
...
def main():
    ...
    report = {
        ...
        "method": (
            f"최신 RSS 뉴스를 클러스터링해 화제 토픽을 뽑고, 정부 예산·조달과 "
            f"연결되는 사안만 골라 {len(findings):,}건의 AI 감사 발견에서 실제 데이터 "
            f"흔적을 교차 확인했습니다. 분석 서술은 Claude Code 가 직접 작성했으며, "
            f"모든 수치는 무죄 추정 하에 '의심 정황'으로 해석해야 합니다."
        ),
